...
echo "###########################"
echo
echo "1. Wait 20 seconds for the mTLS policy to take effect before retrieving the certificate chain of cars POD. As the CA certificate used in this example is self-signed, the verify error:num=19:self signed certificate in certificate chain error returned by the openssl command is expected."
echo "------------------------------------------------------" 
sleep 20

oc exec "$(oc get pod -l app=travels -n $NS_PREFIX-prod-travel-agency -o jsonpath={.items..metadata.name})" -c istio-proxy -n $NS_PREFIX-prod-travel-agency -- openssl s_client -showcerts -connect $(oc -n $NS_PREFIX-prod-travel-agency get svc cars -o jsonpath={.spec.clusterIP}):8000 > cars-cert.txt
