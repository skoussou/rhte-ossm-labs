...
As the communications between RHSSO and `istiod` are secured with a router certificate the Mesh Operator first have to load the certificate to `istiod`. This is performed by the following script:
...
You are ready to test if the external access is secured by sending a request to the _/cars_ and _/travels_ APIs without a JWT Token. This gives as a HTTP 403 Response (RBAC / Access Denied):
