...
** Onboard namespace `dev-travel-agency` to Service Mesh `dev-basic` by adding a `ServiceMeshMember` (`SMM`) resource in `dev-travel-agency` for `dev-travel-agency` namespace to `dev-basic` `ServiceMeshControlPlane` present in `dev-istio-system`.
