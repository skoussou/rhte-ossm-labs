...
* Log in via CLI with user `emma` who has access to `$LAB_PARTICIPANT_ID-dev-istio-system` and get the OCP `ROUTE` to the application business dashboard to verify that you can reach the  *Travel Control Dashboard* in your Browser.
