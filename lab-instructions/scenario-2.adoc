...
* In Kiali as *`mus`* you cannot access the logs of any workload, neither for `istio-proxy` nor for the main workload container. If you select in the menu to the left go to `Workloads`, click on `cars-v1` workload, and go to tab logs, none are displayed. Instead a similar message to the following is displayed
+
