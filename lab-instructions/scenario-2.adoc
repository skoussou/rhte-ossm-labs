...
* You should not be able to modify configs via `Kiali`as user *`mus`*. If you select in the menu to the left `Istio Config`, filter by _Istio Type_ `VirtualService` and enter the `control` config you will notice the config cannot be modified by the user.
+
