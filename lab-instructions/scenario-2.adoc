...
** Onboard namespaces `user$LAB_PARTICIPANT_ID-dev-travel-control` and `user$LAB_PARTICIPANT_ID-dev-travel-portal` to Service Mesh `user$LAB_PARTICIPANT_ID-dev-basic` by adding a `ServiceMeshMember` (`SMM`) resource in each namespace for `user$LAB_PARTICIPANT_ID-dev-basic` membership.
** Deploy the Applications in `user$LAB_PARTICIPANT_ID-dev-travel-control`, `user$LAB_PARTICIPANT_ID-dev-travel-portal` namespaces and `Istio` configs to expose Service Mesh services.
...
* Log in via CLI with user `emma` who has access to `user$LAB_PARTICIPANT_ID-dev-istio-system` and get the OCP `ROUTE` to the application business dashboard to verify that you can reach the  *Travel Control Dashboard* in your Browser.
...
** _control plane_ `user$LAB_PARTICIPANT_ID-dev-istio-system` namespace.
+
...
** As *`farid`*/*`farid`* you will be able to see in `user$LAB_PARTICIPANT_ID-dev-istio-system` the configs but if you try to modify them you will not be able to as you don't have _write_ access in that namespace.
+
image::./assets/02-view-config-but-no-modify.png[400,1000]
+
** If you login in `Kiali` as *`cristina`*/*`cristina`* you should be able to access the configs for the control `VirtualService` and `DestinationRule` in `user$LAB_PARTICIPANT_ID-dev-travel-control` and apply some modification as those namespaces are part of your domain (*Warning:* Don't save any changes).
+
