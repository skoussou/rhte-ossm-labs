...
The following *Purpose* and *Principals* have been finalized with the `Travel Agency` architects and final tuned `Service Mesh` configurations have been accepted based on these:
...
Login as Mesh operator (credentials 'emma/emma') and execute the final update script (on a separate command prompt execute `oc get pods -w -n $LAB_PARTICIPANT_ID-prod-istio-system` to follow the POD scalings).
