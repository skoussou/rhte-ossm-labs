...
We can also login with `farid/farid` in the Openshift Console and verify the applications created in your user-$LAB_PARTICIPANT_ID-prod-travel-agency namespace.
...
* Right now if you login to the Kiali Dashboard with the user `emma/emma`, there is an issue in the `VirtualService` control and link:./assets/link:03-no-gw-for-travel-control-ui-vs.png[an error on Kiali] as no `Gateway` exists yet.
...
https://travel-user-5.apps.cluster-vjzhs.vjzhs.sandbox1672.opentlc.com and the Kiali error on the link:./assets/03-corrected-gw-for-travel-control-ui-vs[`VirtualService` should also show as resolved].
