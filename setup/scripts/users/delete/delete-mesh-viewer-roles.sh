...
#echo "oc adm policy remove-role-from-user view $USERNAME -n $ISTIO_NAMESPACE"
#oc adm policy remove-role-from-user view $USERNAME -n $ISTIO_NAMESPACE
...
for i in "${!array[@]}"
do
    echo "$i=> oc adm policy remove-role-from-user servicemesh-app-viewer $USERNAME -n ${array[i]}"
    oc adm policy remove-role-from-user servicemesh-app-viewer $USERNAME -n ${array[i]}
done
